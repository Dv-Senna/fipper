...
for val in range(amount, -1, -1):
    init_args = ",".join(["AggregateInit{}" for i in range(0, val)])
    content += "\t\ttemplate <typename T>\n"
    content += f"\t\tconsteval auto aggregateSize(AggregateTag<{val}>) -> decltype(T{{{init_args}}}, std::size_t{{}}) {{return {val};}}\n\n"
...
content += f"\t\tstatic constexpr std::size_t value {{__internals::aggregateSize<T> (__internals::AggregateTag<{amount}> {{}})}};\n"
content += "\t};\n\n\n"
